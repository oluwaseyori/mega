...
class Mega:
    ...
    def _download_file(self,
        ...
        if 'g' not in file_data:
            raise RequestError('File not accessible')
        ...
        output_path = Path(dest_path or '') / file_name
        output_path.parent.mkdir(parents=True, exist_ok=True)

        # I made it download with chunked streaming
        with requests.get(file_url, stream=True) as response:
            response.raise_for_status()
            
            # It now initializes crypto
            k_str = a32_to_str(k)
            counter = Counter.new(128, initial_value=((iv[0] << 32) + iv[1]) << 64)
            aes = AES.new(k_str, AES.MODE_CTR, counter=counter)
            
            # Here it uses direct file write
            with open(output_path, 'wb') as f:
                for chunk in response.iter_content(chunk_size=16*1024*1024):  # 16MB chunks
                    if chunk:
                        decrypted = aes.decrypt(chunk)
                        f.write(decrypted)
                        
                        # Tried implementing progress reporting
                        current_bytes = f.tell()
                        if time.time() - last_progress_update > 1:
                            logger.info(
                                f"Progress: {current_bytes/1e9:.2f}GB/"
                                f"{file_size/1e9:.2f}GB "
                                f"({current_bytes/file_size:.1%})"
                            )
                            last_progress_update = time.time()

        # Final file size check mofified my Seyori 
        if output_path.stat().st_size == 0:
            output_path.unlink()
            raise ValueError("Empty file - download failed")
